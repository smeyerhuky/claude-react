...
const AdvancedSpectrogram = () => {
  ...
  // const spectrogramTextureRef = useRef(null); // Reserved for future use
  ...
  const init3DScene = useCallback(() => {
    ...
    // Clean up existing scene if switching modes
  ...
  // Update particle system with new count
  const updateParticleSystem = useCallback((particleCount) => {
  ...
  const calculateMelSpectrogram = (frequencyData) => {
    const { melBins, minFreq, maxFreq } = settings;
  ...
  const updateBlobGeometry = (audioData) => {
    ...
    for (let i = 0; i < positions.count; i++) {
      ...
      const amplitude = audioData[frequency] / 255 * settings.sensitivity;
      const scale = 5 + amplitude * settings.blobTension * 3;
  ...
  const animate = () => {
    ...
    const bassAvg = (bassSum / bassRange / 255) * settings.sensitivity;
    const midAvg = (midSum / (midRange - bassRange) / 255) * settings.sensitivity;
    const highAvg = (highSum / (bufferLength - midRange) / 255) * settings.sensitivity;
    ...
    if (visualMode === 'spectrogram3d' && sceneRef.current) {
  ...
  // 2D visualization
  const draw2DVisualization = (frequencyData) => {
    ...
    ctx.fillStyle = `rgba(0, 0, 0, ${1 - settings.trailLength})`;
    ctx.fillRect(0, 0, width, height);
    
    const colors = colorSchemes[colorScheme];
    
    if (visualMode === 'melSpectrogram') {
      // Draw mel-spectrogram heatmap
      const spectrogramWidth = width / melSpectrogramRef.current.length;
      const binHeight = height / settings.melBins;
  ...
  // Initialize 3D scene when mode changes or component mounts
  useEffect(() => {
    if (isRecording && visualMode === 'spectrogram3d') {
      init3DScene();
    } else if (visualMode !== 'spectrogram3d' && rendererRef.current) {
      // Clean up 3D scene when switching away
  ...
  }, [visualMode, isRecording, init3DScene]);
  ...
  }, [settings.particleCount, isRecording, visualMode, updateParticleSystem]);
...
export default AdvancedSpectrogram;